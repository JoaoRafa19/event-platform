...
export function Subscription () 
{
    ...
    const [craeteSubscriber, {loading}] = useCreateSubscriberMutation();

    async function handleSubscriber (event: FormEvent ) {
        event.preventDefault();
        await craeteSubscriber({variables: {name, email}});
        navigate('/event');  
    }
    ...
    return (
        ...
        <div className="min-h-screen bg-blur bg-cover bg-no-repeat flex flex-col items-center">
            <div className="w-full max-w-[1100px] flex items-center justify-between mt-20 mx-auto">
                ...
                <div className="p-8 bg-gray-700 border border-gray-500 rounded">
                    ...
                    <form onSubmit={handleSubscriber} className="flex flex-col gap-2 w-full " >
                        ...
                        className="bg-gray-900 rounded px-5 h-14"></input>
                        <button
                        disabled={loading} 
                        type="submit" 
            ...
            <img src="/src/assets/image.png" className="mt-10" alt="" ></img>
        </div>
    )
}